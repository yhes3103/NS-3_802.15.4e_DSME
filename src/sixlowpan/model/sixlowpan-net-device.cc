                                      ...
                                      PacketType packetType)
{
    ...
    // howard:
    // 額外印出 payload + 地址資訊
    uint32_t len = copyPkt->GetSize();
    uint8_t buf[200] = {0};
    copyPkt->CopyData(buf, len);
}
...
SixLowPanNetDevice::CompressLowPanHc1(Ptr<Packet> packet, const Address &src, const Address &dst)
{
    ...
    // HC1 僅支援壓縮後 NextHeader = UDP / ICMP / TCP → ns-3內部為簡化  (這裡只示意)
    ...
    NS_LOG_INFO("sixlowpan transmit to low layer packet size: " << packet->GetSize() << " bytes");

    // 回傳「原本 IPv6 header 的大小」(傳統程式碼是回傳這個數值)
