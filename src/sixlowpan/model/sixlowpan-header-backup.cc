...
SixLowPanMesh::Print(std::ostream& os) const
{
    ...
    // hilow
    os << ", intermediate hop:";
    if (Mac64Address::IsMatchingType(m_interHop)) {
        os << Mac64Address::ConvertFrom(m_interHop);
    } else {
        os << Mac16Address::ConvertFrom(m_interHop);
    }
...
SixLowPanMesh::Deserialize(Buffer::Iterator start)
{
    ...
    if (m_hopsLeft >= 0xF) {
        i.Read(buffer, 2);
        m_interHop.CopyFrom(buffer, addrSize);
    }
...
void SixLowPanMesh::SetIntermediateHop(Address hop) {
    m_hilow = true;
    m_interHop = hop;
}

Address SixLowPanMesh::GetIntermediateHop() const {
    return m_interHop;
}
