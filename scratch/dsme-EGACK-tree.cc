 ...
 */
 ...
 int main(int argc, char** argv) {
     ...
     */
             ...
             */
             ...
             for(int slotIdx = 0; slotIdx < 7; slotIdx++)
             {
                 ...
                 else
                 {
                     ...
                     // howard: 猜測用 60 bytes 的封包，throughput 會下降的原因是因為 60 bytes 需要傳比較長的時間
                     // 而當它在 channel 傳輸的時候，GTS time slot 就關閉了，導致 Coord 接收不到這個封包
                     // 這邊把時間加大到 0.00767 似乎還是沒辦法讓 PHY 通知 MAC Layer

                     // 從 Coord Rx 開到 device TX 開會有一段延遲時間，而這個延遲 = device 跳到指定 Channel 所需要花的時間
 
                     // 開 PanID，然後把時間加大到 0.00767 (傳輸到 GTS 的最後一刻)，極限是 55 bytes
                     lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childLrWpanDevIdx), lrwpanDevices.Get(coordLrWpanDevIdx)->GetAddress(),
                                                  pktSize, coord_Rx_GTS_StartTime, device_Tx_GTS_StartTime, 0.006, 0.001);
 
                     // 只傳一個封包的情況
                     // lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childLrWpanDevIdx), lrwpanDevices.Get(coordLrWpanDevIdx)->GetAddress(),
                     //                               pktSize, coord_Rx_GTS_StartTime, device_Tx_GTS_StartTime, 10000, 1);
                 ...
                 coord_Rx_GTS_StartTime += slotTimeInterval; // add for the next GTS slot.
                 device_Tx_GTS_StartTime += slotTimeInterval;
             }
             coord_Rx_GTS_StartTime += (9 * slotTimeInterval); // Because CAP reduction is disabled, we need to calculate next GTS time. Bypass 10 slot time to get the correct GTS time of next superframe.
             device_Tx_GTS_StartTime += (9 * slotTimeInterval);
         }
     ...
     double totalSendSize = (double)(pktRecv * (double)pktSize * 8);  //23040  34560
     double superframeDuration = (double)(960 * 8 / (double)62500);  //0.12288 0.49152
