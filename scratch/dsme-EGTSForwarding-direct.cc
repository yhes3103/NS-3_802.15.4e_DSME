...
//  }
 ...
 int main(int argc, char** argv) {
     ...
     for (unsigned int i = 0; i < lrwpanDevices.GetN(); ++i) 
     {
         ...
         dev->GetMac()->SetEnhancedGTSForwarding(true);
     }
     ...
     // throughput 我沒有跑程式，直接用分析的
     // noGTSForwarding：只能傳兩個封包
     // GTS Forwarding：每個 DSME-GTS 都可以傳 CAP 封包，總共 7 個
     // E-GTS Forwarding：在 0.00768 時槽架構，且上層封包小於 52 bytes 的情況下，每個 DSME-GTS 可以傳兩個 CAP 封包，總共 14 個
