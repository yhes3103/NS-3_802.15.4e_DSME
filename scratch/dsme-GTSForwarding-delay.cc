...
//  }
 ...
 int main(int argc, char** argv) {
     ...
     for (unsigned int i = 0; i < lrwpanDevices.GetN(); ++i) 
     {
         ...
         dev->GetMac()->SetGtsContinuePktSendingFromCap(true);
         
         dev->GetMac()->SetEnhancedGTSForwarding(false);
     }
     ...
     lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(3)->GetObject<LrWpanNetDevice>(), false, 1, channelOffsets[1], 1, 1);  // Devices for TX
     lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(1)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[1], 1, 1);  // Coord for RX
     ...
     // Packet2 1.186623807s received
     // delay = 1.186623807 - 1.175 = 0.011623807
