...
#define NUM_RFD 2 // The number of RFD.
 ...
 // Set a single bit at position X (use shift, not XOR)
 #define BIT(X) (1u << (X))
...
//  }
 ...
 int main(int argc, char** argv) {
    ...
    //  positionAlloc->Add(Vector(-10.0, -10.0, 0.0)); // PAC-C (00:01)
    //  positionAlloc->Add(Vector(10.0, 0.0, 0.0));  // Coord1 (00:02)
    //  positionAlloc->Add(Vector(77.0, -77.0, 0.0));  // Coord2 (00:03)
    //  positionAlloc->Add(Vector(80.0, 80.0, 0.0));  // End device 1 (00:04)
    //  positionAlloc->Add(Vector(95.0, -95.0, 0.0));  // End device 2 (00:05)
    ...
    // NetAnim is initialized later (after devices are installed)
 
     LrWpanHelper lrWpanHelper(true);
     // Add and install the LrWpanNetDevice for each node
    NetDeviceContainer lrwpanDevices = lrWpanHelper.Install(nodes);
    // Enable PCAP tracing for all LR-WPAN devices
    lrWpanHelper.EnablePcapAll(std::string("dsme-data-flooding-simple-mesh-pcap"), true);
    ...
    // channel offset bitmap is a bitmask; set only the bit at panChannelOfs
    hoppingDescriptor.m_channelOfsBitmap = std::vector<uint16_t>(1, static_cast<uint16_t>(BIT(panChannelOfs)));
    ...
    dsmeSuperframeField.SetChannelDiversityMode(CHANNEL_HOPPING);
     ...
     for (unsigned int i = 1; i < NUM_COORD; ++i) {
        ...
        hoppingDescriptor.m_channelOfsBitmapLen = 16;
        // set the bit corresponding to the assigned channel offset
        hoppingDescriptor.m_channelOfsBitmap = std::vector<uint16_t>(1, static_cast<uint16_t>(BIT(channelOffsets[i])));
     ...
     // Ensure end devices (RFDs) synchronize to nearby coordinators and track beacons
     // Node index 3 -> associate and track Coord1 (00:02); Node index 4 -> Coord2 (00:03)
     {
         MlmeSyncRequestParams syncParams;
         syncParams.m_logChPage = 0;
         syncParams.m_trackBcn = true;

         // End device 1 (node 3) tracks Coord1
         Ptr<LrWpanNetDevice> rfd1 = lrwpanDevices.Get(3)->GetObject<LrWpanNetDevice>();
         rfd1->GetMac()->SetAssociatedCoor(Mac16Address("00:02"));
         rfd1->TrackCoordinatorBeacon(syncParams);

         // End device 2 (node 4) tracks Coord2
         Ptr<LrWpanNetDevice> rfd2 = lrwpanDevices.Get(4)->GetObject<LrWpanNetDevice>();
         rfd2->GetMac()->SetAssociatedCoor(Mac16Address("00:03"));
         rfd2->TrackCoordinatorBeacon(syncParams);
     }
    ...
    NetDeviceContainer sixlowDevices = sixlowpan.Install(lrwpanDevices);
     ...
     for(unsigned i = 0; i < sixlowDevices.GetN(); i++)
     {
         ...
         dev->SetHelloMessage(false);
         dev->SetGreedyRouting(false);
         dev->SetLoadRouting(false);
         dev->SetDirect(false);
     }
 
    // Initialize NetAnim after devices exist to ensure callbacks are connected
    AnimationInterface anim("dsme-data-flooding-simple-mesh-pcap.xml");
    anim.SetMobilityPollInterval(Seconds(0.1));
    anim.EnablePacketMetadata(true);
    anim.SetStartTime(Seconds(0.0));
    anim.SetStopTime(Seconds(3.0));
    anim.UpdateNodeDescription(nodes.Get(0), "PAN-C (00:01)");
    anim.UpdateNodeDescription(nodes.Get(1), "Coord1 (00:02)");
    anim.UpdateNodeDescription(nodes.Get(2), "Coord2 (00:03)");
    anim.UpdateNodeDescription(nodes.Get(3), "RFD1 (00:04)");
    anim.UpdateNodeDescription(nodes.Get(4), "RFD2 (00:05)");
    anim.UpdateNodeColor(nodes.Get(0), 255, 0, 0);    // red
    anim.UpdateNodeColor(nodes.Get(1), 0, 128, 255);  // blue-ish
    anim.UpdateNodeColor(nodes.Get(2), 0, 200, 0);    // green
    anim.UpdateNodeColor(nodes.Get(3), 150, 150, 150);// gray
    anim.UpdateNodeColor(nodes.Get(4), 150, 150, 150);// gray
    ...
    // MAC addresses
     ...
     // howard: 00:02 廣播給 00:01 和 00:03，因為 00:02 距離 00:01 比較近，所以 00:01 會先收到封包，之後 00:03 才會收到
     //         有一個問題是 00:01 和 00:03 轉發前都必須用 CSMA/CA 競爭，如果 00:03 拿到的 backoff param 比 00:01 還小
     //         那麼 00:03 會比 00:01 還早傳送，而等 00:03 廣播完後，通道淨空，00:01 才會繼續廣播封包
     //         但理想情況應該是要讓 00:01 比 00:03 先廣播，因為 00:01 比 00:03 先收到封包，這裡算是一個 bug

     // work around:  ns3 預設使用固定種子做亂數，因此每次程式跑出來的亂數都是一樣的
     //               如果用當前時間產生亂數 "有機會" 讓 00:01 的 random backoff time 比 00:03 還小

     // 2025/04/18: 用 2 當亂數種子可以避免這個問題

    // Restore original timing: send at 2s, stop at 3s
    Simulator::Schedule(Seconds(2.0), &SendIPv6Packet, dev0, dstMac);
